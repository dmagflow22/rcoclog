...
const NAV_ITEMS: NavItemDef[] = [
  { href: '/', icon: <Rocket className="h-4 w-4" />, label: 'Deployments', active: true },
  { href: '/projects', icon: <LayoutDashboard className="h-4 w-4" />, label: 'Projects' },
  { href: '/domains', icon: <Globe className="h-4 w-4" />, label: 'Domains' },
  { href: '/logs', icon: <Terminal className="h-4 w-4" />, label: 'Activity' },
        ...
        >
          {!collapsed && (
            <>
              ...
              <span className="flex-1 font-semibold text-sm truncate">DeployWizard</span>
            </>
          )}
          <Tooltip>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-2 py-2 space-y-0.5">
          {!collapsed && (
            <div className="px-3 pt-1 pb-2">
              <Button size="sm" className="w-full justify-start gap-2 h-8 px-2 bg-primary/10 text-primary hover:bg-primary/20 border-none shadow-none">
                <Plus size={14} />
